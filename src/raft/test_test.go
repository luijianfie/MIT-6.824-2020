...
var blockprofile = flag.String("blockprofile", "block.prof", "write block profile to `file`")
var mutexprofile = flag.String("mutexprofile", "mutex.prof", "write mutex profile to `file`")
...
func writeBlockMon() {
	if *blockprofile != "" {
		...
		runtime.GC()                                                // GC，获取最新的数据信息
		if err := pprof.Lookup("block").WriteTo(f, 0); err != nil { // 写入内存信息
...
func writeMutexMon() {
	if *mutexprofile != "" {
		...
		runtime.GC()                                                // GC，获取最新的数据信息
		if err := pprof.Lookup("mutex").WriteTo(f, 0); err != nil { // 写入内存信息
			log.Fatal("could not write block profile: ", err)
		}
		f.Close()
	}
